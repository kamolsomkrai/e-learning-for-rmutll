...
function Imagegallery() {
    const images=[{
      catalog_name: "กิจกรรมวันที่ 1",
      catalog_img: [
        {
          src: 'https://webs.rmutl.ac.th/assets/upload/files/2018/11/20181114112806_98990.png'
        },
        {
    ...
    function showgallery (imagedata){
      return (
        imagedata.map(data=><div><h3>{data.catalog_name}</h3><ResponsiveGallery images={data.catalog_img}/></div>
        )
      )
    }
    
    return (
        <div className="gallery" id="ggggg">
            {showgallery(images)}
        </div>
    )
}
